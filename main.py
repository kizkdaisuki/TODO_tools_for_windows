...
GLOBAL_VAR_DICT_IMPORTANCE = {'low': '🎖', 'mid': '️🎖🎖🎖', 'high': '🎖🎖🎖🎖🎖'}
...
def func_save_to_file(func_param_filepath: str, func_param_file_content: str = ''):
    ...
    if not len(func_param_file_content):
        return
    with open(func_param_filepath, 'a+', encoding='utf-8') as file:
        GLOBAL_VAR_COUNT += 1
...
def func_view_file(func_param_filepath: str):
    with open(func_param_filepath, 'r', encoding='utf-8') as file:
        print(file.read())
        file.close()
...
def func_init_filepath():
    ...
    local_var_json_dict = init_func_read_json('g:/program_code/PY/TODO_tools/todolist/todo.json')
...
def func_init_task_form(func_param_count, func_param_start_time, func_param_end_time, func_param_importance,
    ...
    local_var_first = f'{func_param_count}. [{local_var_time_len}] {func_param_start_time} ~ {func_param_end_time} {GLOBAL_VAR_DICT_IMPORTANCE[func_param_importance]} \n\n'
